...
const updateAllClients = async (io: any) => {
  try {
    ...
    for (let i = 0; i < sockets.length; i++) {
      ...
      if ("token" in loopedSocket.handshake.auth) {
        const verified = await verifyJwt(loopedSocket.handshake.auth.token);
        ...
        const verified = await verifyJwt(loopedSocket.handshake.auth.adminToken);
    ...
    console.error("Error updating all clients:", error);
  }
};

const notifyWebhookUpdate = async (io: any, webhookData: any) => {
  try {
    io.emit("webhookUpdate", webhookData);
  } catch (error) {
    console.error("Error sending webhook update:", error);
  }
};

const notifyRaceUpdate = async (io: any, raceData: any) => {
  try {
    io.emit("raceUpdate", raceData);
  } catch (error) {
    console.error("Error sending race update:", error);
  }
};

const notifyGameStatusUpdate = async (io: any, gameData: any) => {
  try {
    io.emit("gameStatusUpdate", gameData);
  } catch (error) {
    console.error("Error sending game status update:", error);
  }
};
...
export { updateAllClients, notifyWebhookUpdate, notifyRaceUpdate, notifyGameStatusUpdate };
